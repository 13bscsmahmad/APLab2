...
vector<vector<int>> multiplication(vector<vector<int>> &a, vector<vector<int>> &b, vector<vector<int>> &mult, int r1, int c1, int r2, int c2);
...
public:
	int checkMatrixMultiplication(vector<vector<int>> matrixFirst, vector<vector<int>> matrixSecond, vector<vector<int>> resultMatrix, int dimensions) {
		...
		mult.resize(dimensions);
		for (int i = 0; i < dimensions; i++) {
			mult[i].resize(dimensions);
		}

		vector<vector<int>> iterativeMatrix = multiplication(matrixFirst, matrixSecond, resultMatrix, dimensions, dimensions, dimensions, dimensions);
...
int main(int argc, char* argv[])
{
	...
	cout << "Normal multiplication or Strassen? 0 for normal, 1 for Strassen: ";
	cin >> option;
	...
	testMatrix test1;
	if (test1.checkMatrixMultiplication(a, b, mult, r1) == 0) {
...
void take_data(vector<vector<int>> &a, vector<vector<int>> &b, int r1, int c1, int r2, int c2)
{
	...
	for (i = 0; i < a.size(); i++)
		...
		{
			//cout << "a[" << i << "][" << j << "] is being written..." << endl;
			random = rand() % 10 + 1;
			a[i][j] = random;
